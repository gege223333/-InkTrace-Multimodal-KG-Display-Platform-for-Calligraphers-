...
function initInteractions() {
    ...
    if (closeCharacterPanelBtn) {
        closeCharacterPanelBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            e.preventDefault();
            closeCharacterPanel();
        });
    ...
    if (bottomCloseCharacterPanelBtn) {
        bottomCloseCharacterPanelBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            e.preventDefault();
            closeCharacterPanel();
        });
    ...
    // 全屏功能
    const fullscreenCharacterPanelBtn = document.getElementById('fullscreen-character-panel');
    if (fullscreenCharacterPanelBtn) {
        fullscreenCharacterPanelBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            e.preventDefault();
            ...
            if (panel) {
                ...
                if (fullscreenCharacterPanelBtn) {
                    fullscreenCharacterPanelBtn.innerHTML = '⛶';
                    fullscreenCharacterPanelBtn.title = isFullscreen ? '退出全屏' : '全屏';
                }
...
function showPainterPanel(data) {
    ...
    if (pDesc) {
        ...
        if (!isAIResult) {
            ...
            // 尝试从人物文件夹加载版权声明
            const personCopyrightPath = `/assets/人物/图片版权声明.txt`;
            // 尝试从作品图片文件夹加载版权声明
...
function showCharacterPanel(character, images) {
    ...
    if (panel) {
        ...
        // 显示汉字面板时，如果页码显示存在，确保其可见（除非在全屏模式下）
...
function closeCharacterPanel() {
    ...
    // 重置全屏按钮状态
    const fullscreenBtn = document.getElementById('fullscreen-character-panel');
    if (fullscreenBtn) {
