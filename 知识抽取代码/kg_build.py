...
class CalligraphyKGBuilder:
    def __init__(self, api_key: str = DASHSCOPE_API_KEY):
        ...
        if api_key:
            ...
            self.use_api = True
            self.model_name = DASHSCOPE_MODEL
    ...
    def _create_extraction_prompt(self, chunk: Dict) -> str:
        """创建提取提示词 - 针对Qwen模型优化"""
        return f"""你是一个知识图谱专家，专门从中文古籍中提取结构化信息。
...
{{
  "entities": [
    {{
      "name": "实体名称",
      "type": "实体类型(PERSON/SCRIPT_STYLE/WORK/LOCATION/TIME/OTHER)",
      "description": "简要描述",
      "aliases": ["别名1", "别名2"]
    }}
  ],
  "relations": [
    {{
      "subject": "主语实体名称",
      "predicate": "关系类型(创造/命令/属于/位于/师从/同时代)",
      "object": "宾语实体名称",
      "evidence": "原文证据"
    }}
  ],
...
提取要求：
1. 从文本中提取所有重要实体（人物、书体、著作、地点、时间）
2. 提取实体间的关系，特别是"创造"、"命令"等关系
3. 提取重要历史事件
4. 必须返回合法的JSON，不要包含任何额外文本
5. 如果某项没有信息，返回空列表

示例：
文本："太昊伏羲氏，风姓，以木德王。龙马负图，出于荥河，帝则之以画八卦，而文字生焉。"
应返回：
{{
  "entities": [
    {{"name": "太昊伏羲氏", "type": "PERSON", "description": "上古帝王，风姓，以木德王", "aliases": ["伏羲"]}},
    {{"name": "龙马", "type": "OTHER", "description": "祥瑞之兽", "aliases": []}},
    {{"name": "八卦", "type": "WORK", "description": "伏羲所创的八卦图", "aliases": []}},
    {{"name": "荥河", "type": "LOCATION", "description": "河流名称", "aliases": []}}
  ],
  "relations": [
    {{"subject": "太昊伏羲氏", "predicate": "创造", "object": "八卦", "evidence": "帝则之以画八卦"}}
  ],
  "events": []
}}
            ...
            try:
                ...
                if response.status_code == HTTPStatus.OK:
                    ...
                    try:
                        ...
                        if attempt == 0:  # 只在第一次失败时打印前200字符
                            print(f"API返回内容前200字符: {result_text[:200]}")
        ...
        for para in paragraphs:
            ...
            if para_length > CHUNK_SIZE:
                ...
                for sent in sentences:
                    ...
                    if temp_length + sent_length > CHUNK_SIZE and temp_sentences:
                        chunks.append({
                            "chapter": chapter_num,
                            "title": chapter["title"],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="书法史知识图谱构建工具(Qwen版)")
